...
import me.xpyex.plugin.allinone.modelcode.chatgpt.ChatMessage;
...
public final class ChatGPT extends Model {
    private static final WeakHashMap<Long, ChatMessage> CHAT_CACHE = new WeakHashMap<>();
    //private static final String DEFAULT_MSG = "You are an AI chat bot named Icarus who chat with people casually. Always use the same language as user to answer. You are powered by OpenAI and XPYEX.";
    ...
    public void register() {
        registerCommand(Contact.class, new CommandExecutor<>() {
            ...
            public void execute(ContactTarget<Contact> source, ContactTarget<User> sender, String label, String[] args) {
                if (!sender.hasPerm("ChatGPT.use", true)) {
                ...
                if (StringUtil.equalsIgnoreCaseOr(args[0], "talk", "talk4")) {
                    ...
                    if (is3 && !sender.hasPerm("ChatGPT.use.3", true)) {
                        source.sendMessage("你没有使用 ChatGPT 3.5 模型的权限");
                        return;
                    }
                    if (!is3 && !sender.hasPerm("ChatGPT.use.4", false)) {  //调用GPT4且无使用权限，则拦截
